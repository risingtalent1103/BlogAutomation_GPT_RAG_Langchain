...
class ArticleWriter():
    ...
    def create_article_openai(self, topic, debug=False):
        ...
        manual_topic_prompt = PromptTemplate(input_variables=["topic", "topic_direction"], template="""Give me a single, specific topic to write an informative, engaging gold related blog about.
   ...
   The topic should reflect {topic_direction}.
    Only give me the specific topic name after this prompt and nothing else. The topic is:""")
        ...
        topic_directions = ["new idea", "news", "trend", "recent technology", "business strategy", "business idea that addresses a current market need or trend", "concept for a business that leverages emerging technologies or consumer behaviors", "unique business model that disrupts an existing industry", " in-depth analysis of a specific industry, highlighting trends, challenges, and potential opportunities for entrepreneurs", "the impact of technological advancements on traditional business models within a specific sector"]
        logging.info("Loading LLM config")
        ...
        if topic == "":
            ...
            topic_direction_id  =  random.randint(0, len(topic_directions) - 1)
            topic = topic_chain.run({"topic": topic, "topic_direction": topic_directions[topic_direction_id]})
        print('topic: ', topic)
        ...
        if article == None or article == 'None':
